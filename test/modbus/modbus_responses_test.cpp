...
TEST_F(ModbusResponsesTest, readInRegTest_single)
{
    uint16_t addr = 0x0006;
    uint16_t len = 1;
    ...
    EXPECT_CALL(mock, inreg_get(addr))
        .Times(1)
        .WillOnce(Return(0x0102));

    modbus_io_mock_init(&mock);

    modbus_master_read_in_reg(addr, len, requestBuffer);
    modbus_slave_read_inreg_resp(requestBuffer, responseBuffer);

    ASSERT_EQ(MODBUS_FC_READ_IN_REG, responseBuffer[0]);
    ASSERT_EQ(len*2, responseBuffer[1]);
    ASSERT_EQ(read_u16_from_buff(responseBuffer+2), 0x0102);
...
TEST_F(ModbusResponsesTest, readInRegTest_multiple)
{
    uint16_t addr = 0x0006;
    uint16_t len = 3;
    ...
    EXPECT_CALL(mock, inreg_get(addr))
        .Times(1)
        .WillOnce(Return(0xFFFF));
    EXPECT_CALL(mock, inreg_get(addr+1))
        .Times(1)
        .WillOnce(Return(0x01));
    EXPECT_CALL(mock, inreg_get(addr+2))
        .Times(1)
        .WillOnce(Return(0xF00F));
    ...
    ASSERT_EQ(len*2, responseBuffer[1]);
    ASSERT_EQ(read_u16_from_buff(responseBuffer+2), 0xFFFF);
    ASSERT_EQ(read_u16_from_buff(responseBuffer+4), 0x01);
    ASSERT_EQ(read_u16_from_buff(responseBuffer+6), 0xF00F);
